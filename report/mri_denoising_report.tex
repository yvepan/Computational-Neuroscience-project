...
\author{%
课程期末作业：T1 MRI 去噪（T1\_noisy $\rightarrow$ T1\_clean）\\
\texttt{PyTorch 可复现训练、评估与断点续传 Pipeline}
}
...
图~\ref{fig:qual_axial} 展示了测试病例 1000542 的 axial 中层切片。Residual U-Net 降低了 noisy 图像中的随机纹理噪声，同时保留脑室、皮层和深部结构边界。残余误差主要集中在组织边界附近。

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{1000542_axial.png}
  \caption{测试病例 1000542 的 axial 视图：noisy、Residual U-Net 输出、clean 与绝对误差图。}
...
本项目完成了可复现、可断点续传的 T1 MRI 去噪 pipeline。主方法在测试集上达到 PSNR $43.01\pm1.82$ dB、SSIM $0.9959\pm0.0015$，并在所有测试病例上优于 Noisy、Gaussian smoothing 和 NLM。结果表明，2.5D 残差学习能在当前 Gaussian + Rician 混合噪声设置下稳定降低误差。当前实验的主要边界是：模型并非完整 3D 网络，结果基于单一病例级划分，且尚未在外部 MRI 数据上验证。
